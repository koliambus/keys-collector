...
object GitHubSource {
  ...
  def apply(params: List[Map[String, String]]): Source[KeyFinding, Cancellable] = Source
    ...
    .mapAsync(1) { _ =>
      ...
      val responseFuture: Future[List[List[KeyFinding]]] = Future.sequence(params.map(x => {
        Http().singleRequest(
          HttpRequest(uri = SEARCH_URI.withQuery(Query("q" -> x("q"), "access_token" -> Config.GITHUB_TOKEN)))
        ...
        .map(y => y("items").asInstanceOf[List[Object]].map(RepositoryDecoder(x("service"), _)))
      ))
      responseFuture
    }
    .mapConcat(x => x)
